...
function echoStrToLower() {
    echo "${1}" | tr '[:upper:]' '[:lower:]'
}

function addE2EServiceCA() {
    org=`echoStrToLower ${1}`
    sed 's/ORG_NAME/${org}/g' ./templates/docker-compose-e2e-ca.template
}

function addE2EServiceOrderer() {
    org=`echoStrToLower ${1}`
    sed "s/ORG_ORI_NAME/${1}/g" ./templates/docker-compose-e2e-orderer.template | sed "s/ORG_NAME/${org}/g"
}

function addE2EServicePeer() {
    org=`echoStrToLower ${1}`
    cat ./templates/docker-compose-e2e-peer.template
}

# Generate E2E docker compose file
function generateE2EComposeFile() {
    echo "version '2'" > ${COMPOSE_FILE_E2E_NAME}
    echo "volumes:" >> ${COMPOSE_FILE_E2E_NAME}
    echoStrToLower "    orderer0."${ARRAY_ORGS[0]}"."${DOMAIN_NAME} >> ${COMPOSE_FILE_E2E_NAME}
    echoStrToLower "    peer0."${ARRAY_ORGS[0]}"."${DOMAIN_NAME} >> ${COMPOSE_FILE_E2E_NAME}
    echoStrToLower "    peer1."${ARRAY_ORGS[0]}"."${DOMAIN_NAME} >> ${COMPOSE_FILE_E2E_NAME}
    echo "networks:" >> ${COMPOSE_FILE_E2E_NAME}
    echo "    byfn:" >> ${COMPOSE_FILE_E2E_NAME}
    echo "services:" >> ${COMPOSE_FILE_E2E_NAME}
    addE2EServiceOrderer ${ARRAY_ORGS[0]} >> ${COMPOSE_FILE_E2E_NAME}
    # for org in ${ARRAY_ORGS[*]}
    # do
    #     addE2EServiceCA ${org} >> ${COMPOSE_FILE_E2E_NAME}
    #     addE2EServicePeer ${org} >> ${COMPOSE_FILE_E2E_NAME}
    # done 
}
...
function generateCryptoConfig() {
    ...
    if [ ! -f "${CRYPTO_CONFIG_FILE_NAME}" ]
    then
        touch ${CRYPTO_CONFIG_FILE_NAME}
    fi

    # Set orderer nodes
    echo "OrdererOrgs:" > ${CRYPTO_CONFIG_FILE_NAME}
    for org_name in ${ARRAY_ORGS[*]}
    do
        echo "" >> ${CRYPTO_CONFIG_FILE_NAME}
        cat ./templates/crypto-config-orderer.template >> ${CRYPTO_CONFIG_FILE_NAME}

        # Set org name
        sed ${OPTS} "s/ORG_NAME/${org_name}/g" ${CRYPTO_CONFIG_FILE_NAME}
        
        # Set domain name
        DOMAIN=$(echo "${org_name}.${DOMAIN_NAME}" | tr '[:upper:]' '[:lower:]')
        sed ${OPTS} "s/ORG_DOMAIN/${DOMAIN}/g" ${CRYPTO_CONFIG_FILE_NAME}
        # Set orderer hostname
        sed ${OPTS} "s/ORDERER_NUM/0/g" ${CRYPTO_CONFIG_FILE_NAME}
    done

    # Set peer nodes
    echo "" >> ${CRYPTO_CONFIG_FILE_NAME}
    echo "PeerOrgs:" >> ${CRYPTO_CONFIG_FILE_NAME}

    for org_name in ${ARRAY_ORGS[*]}
    do
        echo "" >> ${CRYPTO_CONFIG_FILE_NAME}
        cat ./templates/crypto-config-peer.template >> ${CRYPTO_CONFIG_FILE_NAME}
        ...
        # Set peer count
        sed ${OPTS} "s/PEER_COUNT/2/g" ${CRYPTO_CONFIG_FILE_NAME}

        # Set user count
        sed ${OPTS} "s/USER_COUNT/2/g" ${CRYPTO_CONFIG_FILE_NAME}
    done
}
...
# config file names
CRYPTO_CONFIG_FILE_NAME="crypto-config.yaml"
COMPOSE_FILE_E2E_NAME="docker-compose-e2e.yaml"
#
...
COMPOSE_FILE_E2E=./${COMPOSE_FILE_E2E_NAME}
...
while getopts "h?c:t:d:f:s:l:i:o:n:v" opt; do
    case "$opt" in
    h | \?)
        printHelp
        exit 0
