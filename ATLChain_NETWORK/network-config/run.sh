...
then
    ...
    peer channel update -o ${ORDERER_ADDRESS} -c atlchannel -f OrgAanchors.tx 
    ...
    peer chaincode install -n atlchainCC -v 1.0 -p ATLab-ATLChain
    ...
    peer chaincode instantiate -o ${ORDERER_ADDRESS} -C atlchannel -n atlchainCC -v 1.0 -c '{"Args": ["init"]}' -P "OR('OrgA.peer', 'OrgB.peer')"
    # peer chaincode instantiate -o ${ORDERER_ADDRESS} -C atlchannel -n cc -v 1.0 -c '{"Args":["init","a","100","b","200"]}'
...
then
    echo "### invoke ###"
    peer chaincode invoke -o ${ORDERER_ADDRESS} -C atlchannel -n atlchainCC -c '{"Args":["Put", "tryPutkey", "{\"tryAddrReceive\":\"trytestAddrA\", \"tryAddrSend\":\"trytestAddrB\"}", "trysignagure", "trypubKey"]}'
    # peer chaincode invoke -o ${ORDERER_ADDRESS} -C atlchannel -n cc -c '{"Args":["invoke","a","b","1"]}' 
